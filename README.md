...
> The baked pivot data is stored in UV channels (and optionally in the vertex color) so it can be accessed in the target engine's shader.

## Advanced Geometry Nodes

The **Advanced** section allows experienced users to load the Geometry Nodes used by Auto Pivot Baker. This can be used to create custom workflows, store additional per-instance data in the vertex color, or modify the baked data before exporting. However this means that you need to know how to correctly export meshes and data for you target game engine.
